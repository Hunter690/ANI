...
def propagate(w, b, X, Y):
    ...
    ### START CODE HERE ### (≈ 2 lines of code)
    A = sigmoid(np.dot(w.T, X) + b) # A.shape = (1, number_of_examples)
    A_log = np.log(A)
    #print('A = ', A)
    #print('A_log = ', A_log)
    loss = Y * A_log + (1-Y) * np.log(1-A)
    #print('loss = ', loss)
    #print('-1/m = ', -1/m)
    ...
    ### START CODE HERE ### (≈ 2 lines of code)
    dJ_over_dw = 1/m * np.dot(X, (A - Y).T)
    dJ_over_db = 1/m * np.sum(A - Y)
    ### END CODE HERE ###

    assert(dJ_over_dw.shape == w.shape)
    assert(dJ_over_db.dtype == float)
...
```python
d = model(train_set_x, train_set_y, test_set_x, test_set_y, num_iterations = 2000, learning_rate = .005, print_cost = True)
```
